...
- `entrypoint.sh` — Fish shell entrypoint that runs `claude --dangerously-skip-permissions`, optionally notifies via ntfy.sh on exit.
...
- **No Claude/bun in image** — both are picked up from the host home mount. Entrypoint uses fish shell to inherit the user's PATH config.
