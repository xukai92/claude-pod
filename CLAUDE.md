...
- **macOS support** — on Darwin, `--userns=keep-id` and `--security-opt label=disable` are skipped; `podman machine` is auto-started; `HOME_DIR` build arg ensures the container user's home matches the host (e.g. `/Users/kai`).
